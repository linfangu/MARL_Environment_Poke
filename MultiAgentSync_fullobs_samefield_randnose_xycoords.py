...
class MultiAgentSync_fullobs(MultiAgentEnv):
    ...
    def _get_obs(self):
        """
        Returns observation for each agent as a one-hot encoded flat vector,
        matching the Box observation space.
        """

        # 10 discrete indices for agent1
        agent1_vals = [
            self.poke_coords1[1],              # nosepoke1
            self.water_coords1[1],             # water1
            self.agent2_pos[1] - 4,            # otheragent1
            self.agent1_pos[1],                # self1
            self.poke_coords2[1] - 4,          # otherpoke1
            self.poke_coords1[0],              # nosepoke0
            self.water_coords1[0],             # water0
            self.agent2_pos[0],                # otheragent0
            self.agent1_pos[0],                # self0
            self.poke_coords2[0],              # otherpoke0
        ]

        # 10 discrete indices for agent2
        agent2_vals = [
            self.poke_coords2[1] - 4,          # nosepoke1
            self.water_coords2[1] - 4,         # water1
            self.agent1_pos[1],                # otheragent1
            self.agent2_pos[1] - 4,            # self1
            self.poke_coords1[1],              # otherpoke1
            self.poke_coords2[0],              # nosepoke0
            self.water_coords2[0],             # water0
            self.agent1_pos[0],                # otheragent0
            self.agent2_pos[0],                # self0
            self.poke_coords1[0],              # otherpoke0
        ]

        def one_hot_encode(values, nvec):
            # Returns a 1D flat list of all one-hot vectors concatenated
            encoded = []
            for val, dim in zip(values, nvec):
                vec = [0.0] * dim
                vec[int(val)] = 1.0
                encoded.extend(vec)
            return encoded

        agent1_obs = one_hot_encode(agent1_vals, self.nvec)
        agent2_obs = one_hot_encode(agent2_vals, self.nvec)

        return {
            "agent1": agent1_obs,  # Flat list of floats, matching Box space shape
            "agent2": agent2_obs,
        }
...
class MultiAgentSing_fullobs(MultiAgentSync_fullobs):
    def __init__(self, config=None):
